...
> ⚠️ 以下指标为 2025-11-16 11:20-11:26（本地时间）在默认配置、保留全部特征情况下执行 所得基线。默认为成本策略（`fn_cost=5`，`fp_cost=1`）扫描 201 个阈值。
...
### 2025-11-16 16:27 + 阈值成本调整（FN:FP = 2:1）

> 调整 `threshold_tuning.fn_cost` 由 5.0 降至 2.0，重新执行 `python3 -m ai_homework.cli.run_pipeline --skip-data`，数据与模型配置保持一致。

#### 1.关键观察

- 阈值整体右移：Logistic 0.41、LightGBM 0.536、XGBoost 0.545、CatBoost 0.6845，明显高于先前 5:1 成本下的阈值（0.08~0.38），模型开始放行更多好客户。
- Precision 均提升至 0.50~0.53 区间，Recall 回落至 0.74~0.83；验证集 expected_cost 下降到约 5.7k（此前约 6.5k），印证“FN 成本降低”后整体业务损失随之下降。
- CatBoost 仍为首选：测试集 Precision 0.532、Recall 0.747、F1 0.621、expected_cost ≈ 5.74k，并保持最高的 ROC-AUC≈0.686；LightGBM 在 Recall 方面相对突出（0.816）。
- Logistic Regression 在高阈值下仍保持 0.786 的 Recall，说明概率分布右移后仍能覆盖大部分逾期合同，适合作为可解释基线。

#### 2.阈值与成本摘要

- 仍采用 `strategy=cost` + 201 点均匀网格；新的成本权重 (2:1) 下，四个模型的验证集 expected_cost 分别为 5.89k / 5.73k / 5.71k / 5.74k。
- 完整阈值搜索轨迹与指标记录：`outputs/artifacts/{model}_threshold_search.csv`、`outputs/artifacts/summary_{model}.json`。

#### 3.测试集指标对比

| 模型                | Accuracy        | Precision       | Recall | F1              | Expected Cost   | 阈值   |
| ------------------- | --------------- | --------------- | ------ | --------------- | --------------- | ------ |
| Logistic Regression | 0.579           | 0.506           | 0.786  | 0.615           | 5.91k           | 0.41   |
| LightGBM            | 0.580           | 0.506           | 0.816  | 0.625           | 5.74k           | 0.536  |
| XGBoost             | 0.571           | 0.500           | 0.830  | 0.624           | 5.77k           | 0.545  |
| CatBoost            | **0.610** | **0.532** | 0.747  | **0.621** | **5.74k** | 0.6845 |

> 数据来源：`outputs/reports/tables/model_metrics.csv`（run_identifier=`model_training_20251116_162130`）。

#### 4.可视化与产物

- 新阈值下的混淆矩阵、ROC、SHAP 图已覆盖在 `outputs/reports/figures/`；与旧版对比可见负类预测比例上升（左上角 TN 增大）。
- 对应模型文件与预测结果：`outputs/models/{model}_20251116_1623*.joblib`、`outputs/artifacts/test_predictions_{model}.parquet` 等。

##### 4.1 混淆矩阵解读（测试集）

![Logistic Regression CM (2025-11-16 16:27)](../outputs/reports/figures/confusion_matrix_logistic_regression_20251116_1627.png)
![LightGBM CM (2025-11-16 16:27)](../outputs/reports/figures/confusion_matrix_lightgbm_20251116_1627.png)
![XGBoost CM (2025-11-16 16:27)](../outputs/reports/figures/confusion_matrix_xgboost_20251116_1627.png)
![CatBoost CM (2025-11-16 16:27)](../outputs/reports/figures/confusion_matrix_catboost_20251116_1627.png)

- 四个模型的左上角（TN）均较基线显著增加，以 CatBoost 为例 TN≈3.33k（原先不足 200 条），说明自动放行的正常客户数量明显上升。右上角（FP）仍是主要成本来源，但数量已从 ~6.5k 降至 ~3.2k~3.9k。
- CatBoost 与 LightGBM 的右下角（TP）保持在 3.6k~3.8k，漏掉的坏客户（左下角 FN）分别约 1.25k 与 0.90k，换算成成本后与表格中的 expected cost 对应。
- Logistic Regression 的 TP 数最少（与其较低的 recall 对应），但 FP 也最低，体现其作为线性基线的稳健表现。
- 若以“无模型时全部人工审核”为对照，以 CatBoost 为例可直接放行约 3.3k 笔低风险合同，人工只需聚焦模型判定的约 6.9k 高危样本（TP+FP≈3.7k+3.2k），审核工作量下降超过一半。即便仍有 1.25k 左右 FN 需要贷后补救，也远低于完全人工筛查时的漏判风险，业务可据此规划“自动放行-人工复核-重点干预”的分层策略。

##### 4.2 ROC-AUC 对比

![Logistic Regression ROC (2025-11-16 16:27)](../outputs/reports/figures/roc_curve_logistic_regression_20251116_1627.png)
![LightGBM ROC (2025-11-16 16:27)](../outputs/reports/figures/roc_curve_lightgbm_20251116_1627.png)
![XGBoost ROC (2025-11-16 16:27)](../outputs/reports/figures/roc_curve_xgboost_20251116_1627.png)
![CatBoost ROC (2025-11-16 16:27)](../outputs/reports/figures/roc_curve_catboost_20251116_1627.png)

- AUC 仍维持在 0.668~0.687 区间，说明排序能力基本未受成本调整影响；CatBoost 以 ~0.686 稍胜。
- LightGBM 与 XGBoost 的 ROC 曲线在低 FPR 区域稍高，意味着如果未来继续提高阈值，这两类树模型仍可提供较高的召回。

##### 4.3 SHAP 主导特征

![LightGBM SHAP (2025-11-16 16:27)](../outputs/reports/figures/shap_summary_lightgbm_20251116_1627.png)
![XGBoost SHAP (2025-11-16 16:27)](../outputs/reports/figures/shap_summary_xgboost_20251116_1627.png)
![CatBoost SHAP (2025-11-16 16:27)](../outputs/reports/figures/shap_summary_catboost_20251116_1627.png)

- 三个树模型的 SHAP Top 特征基本一致：历史逾期相关指标（`history_overdue_terms`、`history_overdue_rate`）与本期额度特征（`loan_amount`, `outstanding_principal`）位列前茅，说明模型仍依赖历史行为判断风险。
- LightGBM 的 SHAP 分布显示较高额度与较长期限对风险有明显正贡献；CatBoost 由于类别处理能力突出，`rating_numeric`、`loan_type_*` 的影响更清晰。
- 调整阈值后 SHAP 排名未发生结构性变化，证明模型学习到的风险信号稳定，可作为后续特征优化与策略设计的依据。

## 3. 产物索引

- **运行日志**
  - `outputs/logs/model_training.log`：完整记录模型训练、阈值调优、产物落盘路径。
  - `outputs/logs/data_preparation.log`：若同时执行数据准备流水线，可回溯数据清洗与特征构建细节。
- **模型与核心产物**
  - `outputs/models/{model}_{timestamp}.joblib`：按时间戳保存的可复用模型文件。
  - `outputs/artifacts/summary_{model}.json`：封装本次 run 的最佳参数、阈值、三集合指标与配置快照。
  - `catboost_info/`：CatBoost 训练生成的原生日志（迭代指标、时间预估），需保留备查。
- **调参与阈值搜索**
  - `outputs/artifacts/{model}_tuning_history.csv`：超参搜索轨迹（含交叉验证得分）。
  - `outputs/artifacts/{model}_threshold_search.csv`：阈值网格扫描明细（可能因配置跳过）。
  - `outputs/reports/tables/threshold_summary.csv`：各模型调优后阈值与成本指标的汇总表。
- **预测结果与数据导出**
  - `outputs/artifacts/test_predictions_{model}.parquet`：测试集逐笔预测（ID、真值、标签、概率）。
  - `data/processed/{train,valid,test}.parquet`：为训练流程生成的特征集（来自 `prepare_data` 流水线）。
  - `outputs/artifacts/numeric_scaler.pkl`：数值特征标准化器（含字段清单），推理阶段复用。
- **评估指标与解释性数据**
  - `outputs/reports/tables/model_metrics.csv|json`：训练/验证/测试全量指标、阈值、成本。
  - `outputs/reports/tables/feature_importance_{model}.csv`：树模型的特征重要性（Logistic 无该产物）。
  - `outputs/reports/tables/shap_values_{model}.csv`：若 `evaluation.shap.enabled=true` 且模型支持树解释，将导出样本级 SHAP。
  - `outputs/reports/tables/feature_selection_summary.csv`：在启用特征选择时记录入选特征与评分。
  - `outputs/reports/tables/feature_analysis_summary.csv`、`outputs/reports/tables/feature_high_correlation.csv`：数据探索统计（取决于 `feature_analysis` 配置）。
- **图形报告**
  - `outputs/reports/figures/confusion_matrix_{model}.png` & `confusion_matrix_{model}_{run_identifier}.png`：基线与每次运行的混淆矩阵。
  - `outputs/reports/figures/roc_curve_{model}.png`：ROC 曲线对比。
  - `outputs/reports/figures/shap_summary_{model}.png`：树模型的 SHAP Top 特征条形/散点摘要。
